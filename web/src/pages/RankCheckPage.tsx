...
import { FiChevronDown, FiChevronRight, FiTrash2, FiTrendingUp, FiTrendingDown, FiMinus, FiClock, FiMousePointer } from 'react-icons/fi';
...
interface ClickStats {
  ...
  dayBefore: number;
  thisWeek: number;
  todayPerKeyword: { itemName: string; keyword: string; count: number }[];
}

interface ClickHistoryEntry { date: string; count: number; }
...
function ProductRow({ group, onShowHistory, onShowClickHistory }: {
  group: GroupedProduct;
  onShowHistory: (itemName: string, keyword: string) => void;
  onShowClickHistory: (itemName: string, keyword: string) => void;
}) {
  ...
  const avgRank = trackedKeywords.length > 0
    ...
    : null;
  const totalClicks = group.keywords.reduce((s, k) => s + k.todayClicks, 0);
    ...
    <Box borderWidth="1px" borderRadius="lg" mb={3} bg="white">
      <Flex px={4} py={3} cursor="pointer" onClick={onToggle} align="center" _hover={{ bg: 'gray.50' }} borderBottomWidth={isOpen ? '1px' : 0}>
        ...
        <HStack spacing={4}>
          ...
          {avgRank != null && <Badge colorScheme="blue" fontSize="sm">평균 {avgRank}위</Badge>}
            ...
            <Tbody>
              {group.keywords.map((k) => (
                <Tr key={`${k.itemName}-${k.keyword}`} bg={!k.rank ? 'gray.50' : undefined}>
                  ...
                  <Td>
                    {k.todayClicks > 0 ? (
                      <Badge colorScheme="teal" fontSize="xs">{k.todayClicks}회</Badge>
                    ) : (
                      <Text fontSize="xs" color="gray.400">0</Text>
                    )}
                  </Td>
                  <Td fontSize="xs" color="gray.500">
                    {k.rank ? new Date(k.rank.checkedAt).toLocaleString('ko-KR', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}
                  </Td>
                  <Td>
                    <HStack spacing={1}>
                      {k.rank ? (
                        <Button size="xs" variant="ghost" leftIcon={<FiClock />} onClick={() => onShowHistory(k.itemName, k.keyword)}>순위</Button>
                      ) : (
                        <Text fontSize="xs" color="gray.400">-</Text>
                      )}
                      <Button size="xs" variant="ghost" leftIcon={<FiMousePointer />} onClick={() => onShowClickHistory(k.itemName, k.keyword)}>클릭</Button>
                    </HStack>
                  </Td>
                </Tr>
              ))}
...
export default function RankCheckPage() {
  ...
  const [clickHistoryModal, setClickHistoryModal] = useState<{ itemName: string; keyword: string; items: ClickHistoryEntry[] } | null>(null);
  ...
  const loadClickStats = useCallback(async () => {
    try {
      const res = await fetch('/api/click-stats', { credentials: 'include' });
  ...
  useEffect(() => {
    ...
    loadRanks();
    loadClickStats();
  }, [loadKnowledges, loadRanks, loadClickStats]);
  ...
  const clearAll = async () => {
    if (!confirm('모든 순위 및 클릭 기록을 삭제하시겠습니까?')) return;
    await api.rankChecks.clear();
    setRanks([]);
    setAllHistory(new Map());
    setClickStats({ today: 0, yesterday: 0, dayBefore: 0, thisWeek: 0, todayPerKeyword: [] });
  };
  ...
  const showClickHistory = async (itemName: string, keyword: string) => {
    try {
      const res = await fetch(`/api/click-stats/history?itemName=${encodeURIComponent(itemName)}&keyword=${encodeURIComponent(keyword)}`, { credentials: 'include' });
      const data = await res.json();
      setClickHistoryModal({ itemName, keyword, items: data.items });
  ...
  for (const r of ranks) rankMap.set(`${r.itemName}::${r.keyword}`, r);

  const clickMap = new Map<string, number>();
  for (const c of clickStats.todayPerKeyword) clickMap.set(`${c.itemName}::${c.keyword}`, c.count);
  ...
  for (const k of knowledges) {
    ...
    allKeywordRanks.push({
      keyword: k.keyword, itemName: k.itemName, purchaseName: k.purchaseName,
      groupName: k.groupName, rank: currentRank, yesterdayRank: yesterdayRecord, trend,
      todayClicks: clickMap.get(rankKey) ?? 0,
    });
  }
  ...
  for (const k of filteredKeywordRanks) {
    ...
    if (!group) {
      group = { itemName: k.itemName, purchaseName: k.purchaseName, groupName: k.groupName, keywords: [] };
  ...
  return (
    <Stack spacing={5}>
      <Flex justify="space-between" align="center">
        ...
        <Button leftIcon={<FiTrash2 />} size="sm" variant="outline" colorScheme="red" onClick={clearAll} isDisabled={ranks.length === 0}>
          기록 삭제
        </Button>
      </Flex>
      ...
      <Box bg="blue.50" borderRadius="md" p={3}>
        <Text fontSize="sm" color="blue.700">
          워커 PC가 크롤링할 때 자동으로 상품 순위와 클릭 수가 기록됩니다.
        </Text>
      </Box>
      ...
      <SimpleGrid columns={{ base: 2, md: 4 }} spacing={4}>
        <Stat bg="white" borderWidth="1px" borderRadius="lg" p={4} borderLeftWidth="4px" borderLeftColor="teal.400">
          <StatLabel>
            <HStack spacing={1}><Icon as={FiMousePointer} color="teal.500" /><Text>오늘 클릭</Text></HStack>
          </StatLabel>
          <StatNumber color="teal.600">{clickStats.today}</StatNumber>
        </Stat>
        <Stat bg="white" borderWidth="1px" borderRadius="lg" p={4}>
          <StatLabel>어제 클릭</StatLabel>
          <StatNumber>{clickStats.yesterday}</StatNumber>
        </Stat>
        <Stat bg="white" borderWidth="1px" borderRadius="lg" p={4}>
          <StatLabel>그저께 클릭</StatLabel>
          <StatNumber>{clickStats.dayBefore}</StatNumber>
        </Stat>
        <Stat bg="white" borderWidth="1px" borderRadius="lg" p={4} borderLeftWidth="4px" borderLeftColor="purple.400">
          <StatLabel>이번주 합계</StatLabel>
          <StatNumber color="purple.600">{clickStats.thisWeek}</StatNumber>
        </Stat>
      </SimpleGrid>
      ...
      <SimpleGrid columns={{ base: 2, md: 4 }} spacing={4}>
        ...
        <Stat {...statCardStyle(filter === 'outOfRank', 'orange')} onClick={() => toggleFilter('outOfRank')}>
          <StatLabel><HStack spacing={1}><Text>순위 밖</Text></HStack></StatLabel>
      ...
      {grouped.length > 0 ? (
        grouped.map((g) => (
          <ProductRow key={g.itemName} group={g} onShowHistory={showHistory} onShowClickHistory={showClickHistory} />
        ))
      ) : (
        <Box borderWidth="1px" borderRadius="lg" p={8} textAlign="center">
          <Text color="gray.500">
            {filter ? '해당 필터에 맞는 키워드가 없습니다.' : '등록된 키워드가 없습니다.'}
          </Text>
        </Box>
      )}
      ...
      <Modal isOpen={!!clickHistoryModal} onClose={() => setClickHistoryModal(null)} size="md">
        ...
        <ModalContent>
          <ModalHeader>
            ...
            <Text fontSize="sm" color="gray.500" fontWeight="normal">상품번호: {clickHistoryModal?.itemName}</Text>
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
