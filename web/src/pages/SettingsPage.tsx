    ...
    <Stack spacing={6}>
      ...
      <Box borderWidth="1px" borderRadius="lg" p={6}>
        <Stack spacing={6}>
          ...
          {/* ── 호스트 IP (vmMode === 'VM') ── */}
          {s.vmMode === 'VM' && (
            <Section title="호스트 설정">
              <Field label="호스트머신 IP주소">
                <Input value={s.hostIpAddress} onChange={(e) => set('hostIpAddress', e.target.value)} placeholder="예: 192.168.0.10" />
              </Field>
            </Section>
          )}
          ...
          <Section title="VM 모드">
            <Field label="VM 모드">
              <RadioGroup value={s.vmMode} onChange={(v) => set('vmMode', v as Settings['vmMode'])}>
                <HStack spacing={5}>
                  <Radio value="X">단일 머신</Radio>
                  <Radio value="본컴">본컴 (Host)</Radio>
                  <Radio value="VM">게스트 (VM)</Radio>
                </HStack>
              </RadioGroup>
            </Field>
          </Section>
