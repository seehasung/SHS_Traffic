...
// CLI로 실행: node worker-client.js --server http://HOST:17321 --id worker1 --pw pass123
...
const serverUrl = getArg('--server') || process.env.WORKER_SERVER_URL || '';
const loginId = getArg('--id') || process.env.WORKER_LOGIN_ID || '';
const loginPassword = getArg('--pw') || process.env.WORKER_LOGIN_PASSWORD || '';

if (!serverUrl || !loginId || !loginPassword) {
  console.error('사용법: node worker-client.js --server http://HOST:17321 --id WORKER_ID --pw PASSWORD');
  console.error('또는 환경변수: WORKER_SERVER_URL, WORKER_LOGIN_ID, WORKER_LOGIN_PASSWORD');
  process.exit(1);
}

const client = new WorkerClient({ serverUrl, loginId, loginPassword });
client.start();
